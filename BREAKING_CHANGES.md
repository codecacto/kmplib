...
|--------|--------|----------|----------|
| 2.120.0 | `ui/screens/login` | `LoginEffect.Navigate` ganhou **`ToForgotPassword`**. Só quebra quem tem **`when (effect)` exaustivo sem `else`** sobre os efeitos do login — que é o padrão das `Route` da fábrica (a casca-mobile acusou no primeiro build). Nada mais muda: quem usa o diálogo embutido segue ligando `showForgotPasswordDialog`. | Acrescentar o ramo. App com TELA própria de recuperação: navegue nele e passe o `LoginViewModel` a **emitir o efeito** em vez de ligar o flag do diálogo (era o que fazia o modal piscar). App que usa o diálogo da lib: `-> Unit`, com um comentário dizendo por quê. |
